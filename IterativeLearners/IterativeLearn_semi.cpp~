...
void IterativeLearn_semi::get_initial_edges(std::vector<unsigned int>& new_idx){
    ...
    read_all_edge_features(all_features, all_labels);
...
//     double w_dist_thd = 7;//7;
//     wt2 = new WeightMatrix2(w_dist_thd, ignore_list);
//     wt2->weight_matrix_parallel(all_features, false);
    
    printf("computing wtmat with thd= %.4lf\n",w_dist_thd);
    wt1 = new WeightMatrix_iter(2, w_dist_thd, ignore_list);
...
// //     eclfr->learn(cum_train_features, cum_train_labels); // number of trees
    eclfr->learn(cum_train_features, cum_train_labels); // number of trees


//     wt2->solve(prop_lbl);
    std::map<unsigned int, vector<double> > prop_lblv;
    wt1->solve(prop_lblv);
    ...
    std::map<unsigned int, std::vector<double> >::iterator plit;
    for(plit = prop_lblv.begin(); plit != prop_lblv.end(); plit++){
...
// // 	double rf_p2 = eclfr->predict(all_features[idx]);
	double rf_p1 = eclfr->predict(all_features[idx]);
	...
	prop_lbl.insert(std::make_pair(idx,pp));
...
//     wt1->add2trnset(trn_idx, cum_train_labels);
    wt1->add2trnset(new_idx, new_lbl);
...
// 	for(size_t ii=0; ii < feat2add && rit != risks.rend() ; ii++, rit++){
    ...
    //*C* debug
    FILE* fp= fopen("selected_trnidxC.txt","wt");
